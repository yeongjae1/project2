...
public class NoticeController {
	...
	public String Index(HttpServletRequest request, HttpServletResponse response, 
		...
		List<Yes24DTO> qList = Yes24Service.bookList();
		...
		//List<KyoboDTO> bList = aList.subList(1, 100);
	...
	@RequestMapping(value = "book/tts")	
	public String tts(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {

		log.info(this.getClass());
		List<KyoboDTO> rList = kyoboService.tts();
		List<Yes24DTO> qList = Yes24Service.tts();
		if (rList==null){
			rList = new ArrayList<KyoboDTO>();
		}
		if (qList==null){
		...
		for(int i = 0; i < 10; i++) {
			log.info("########"+rList.get(i).getTitle());
			log.info("########"+qList.get(i).getTitle());
		}
		model.addAttribute("rList", rList);
		model.addAttribute("qList", qList);
		rList = null;
	...
	public String search(HttpServletRequest request, HttpServletResponse response,
		...
		List<KyoboDTO> rList = kyoboService.search(pMap);
		
		qMap.put("category", category);
		qMap.put("search", search);
		List<Yes24DTO> qList = Yes24Service.search(qMap);
		model.addAttribute("rList",rList);
		model.addAttribute("qList",qList);
		return "/book/search";
	}
	@RequestMapping(value = "book/random")	
	public String random(HttpServletRequest request, HttpServletResponse response,
			ModelMap model) throws Exception {

		log.info(this.getClass() + ".random start!");
		List<KyoboDTO> rList = kyoboService.random();
		List<Yes24DTO> qList = Yes24Service.bookList();
		...
		rList = null;
		qList = null;
		log.info(this.getClass() + ".random end!");
		return "/book/random";
	}
	/**
	 * 게시판 리스트 보여주기
